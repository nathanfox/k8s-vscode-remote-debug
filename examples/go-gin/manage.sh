...
readonly APP_NAME="go-gin"
readonly IMAGE_NAME="go-gin"
...
show_help() {
    cat << EOF
Go Gin - Management Script v${VERSION}
...
COMMANDS:
    build                      Build Docker image (with Delve debugger)
    push                       Push image to registry
    deploy                     Deploy to namespace
    debug                      Verify pod ready and port-forward debug port (2345)
    port-forward [PORT]        Port-forward app port (default: 8080)
    port-forward-debug         Port-forward debug port (2345)
...
cmd_debug() {
    ...
    log_info "Setting up Go debugging with Delve..."
    ...
    log_info ""
    log_info "Port-forwarding Delve debug port 2345..."
    port_forward_pod "$NAMESPACE" "$pod_name" "2345" "2345"
}
