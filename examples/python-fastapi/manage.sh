...
readonly APP_NAME="python-fastapi"
readonly IMAGE_NAME="python-fastapi"
...
show_help() {
    cat << EOF
Python FastAPI - Management Script v${VERSION}
...
COMMANDS:
    ...
    debug                      Verify pod ready and port-forward debug port (5678)
    port-forward [PORT]        Port-forward app port (default: 8080)
    port-forward-debug         Port-forward debug port (5678)
...
EXAMPLES:
    ...
    # Debug (port-forward debugger port 5678)
    $SCRIPT_NAME -n dev-yourname debug
    # Then attach VS Code debugger (F5)

    # Debug specific pod (if multiple replicas)
    $SCRIPT_NAME -n dev-yourname -p python-fastapi-7b4465f95-s94w2 debug
...
cmd_debug() {
    ...
    log_info "Setting up Python debugging..."
    ...
    log_info ""
    log_info "Port-forwarding debug port 5678..."
    port_forward_pod "$NAMESPACE" "$pod_name" "5678" "5678"
}
