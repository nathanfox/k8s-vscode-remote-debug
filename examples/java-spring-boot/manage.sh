...
show_help() {
    cat << EOF
Java Spring Boot - Management Script v${VERSION}
...
COMMANDS:
    build                      Build Docker image (with JDWP debugger)
    push                       Push image to registry
    deploy                     Deploy to namespace
    debug                      Verify pod ready and port-forward debug port (5005)
    port-forward [PORT]        Port-forward app port (default: 8080)
    port-forward-debug         Port-forward debug port (5005)
